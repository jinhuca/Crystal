...
namespace Crystal.Provider.Telemetry;
...
internal sealed class RAMSPDToolkitDriver : IPawnIODriver {
  ...
  public IPawnIOModule LoadModule(PawnIOSMBusIdentifier pawnIOSMBusIdentifier) {
    ...
    try {
      var pawnIO = PawnIo.PawnIo.LoadModuleFromResource(typeof(RAMSPDToolkitDriver).Assembly, $"{nameof(Crystal.Provider.Telemetry)}.Resources.PawnIo.{moduleResourceFilename}");
