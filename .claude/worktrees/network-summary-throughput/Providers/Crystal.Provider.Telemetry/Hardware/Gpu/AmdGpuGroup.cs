using Crystal.Provider.Telemetry.Interop;
using System;
...
namespace Crystal.Provider.Telemetry.Hardware.Gpu;
...
internal class AmdGpuGroup : IGroup {
  ...
  public AmdGpuGroup(ISettings settings) {
    try {
      // Pre-flight probe so a machine without the AMD driver (atiadlxx.dll absent, e.g. any
      // non-AMD GPU) short-circuits here instead of hard-faulting on the first [DllImport]. This
      // mirrors the DllExists()/*_Method_Exists guard the NVIDIA, Intel and FTDI paths already use;
      // AMD was the only group reaching a raw P/Invoke, which is what surfaced the (caught, benign)
      ...
      if (!AtiAdlxx.ADL_Method_Exists(nameof(AtiAdlxx.ADL2_Main_Control_Create))) {
        ...
        _report.AppendLine();
        _report.AppendLine("Status: atiadlxx.dll not available");
        return;
      }
