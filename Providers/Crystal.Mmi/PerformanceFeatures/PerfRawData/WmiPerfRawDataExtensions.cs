...
namespace Crystal.Mmi.PerformanceFeatures.PerfRawData;

public static class WmiPerfRawDataExtensions {
  private const string WmiClassName = "Win32_PerfRawData";

  public static async Task<IReadOnlyList<PerfRawDataMetrics>> ToSafePerfRawDataMetricsAsync(
      this IWmiHardwareProvider provider,
      CancellationToken cancellationToken) {
    try {
      var instancesData = await provider.GetMultiMetricsForClassAsync(WmiClassName, cancellationToken);
      if (instancesData == null || instancesData.Count == 0) return Array.Empty<PerfRawDataMetrics>();

      var results = new List<PerfRawDataMetrics>(instancesData.Count);
      ...
      foreach (var data in instancesData) {
        ...
        results.Add(new PerfRawDataMetrics(
      ...
      return Array.Empty<PerfRawDataMetrics>();
    }
  }
}
