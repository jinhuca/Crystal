...
public class TemperatureProbeInformationTests {
  ...
  public void Decode_ValidTemperatureProbe_ParsesFieldsAndSplitsStatusByte() {
    ...
    rawBytes[0x07] = 0x52; rawBytes[0x08] = 0x03;
    // Min Value: -10.0°C -> -100 in tenths -> Two's complement 0xFF9C
    rawBytes[0x09] = 0x9C; rawBytes[0x0A] = 0xFF;
    // Resolution: 0.5°C -> 5 in tenths -> 0x0005
    rawBytes[0x0B] = 0x05; rawBytes[0x0C] = 0x00;
    // Tolerance: 2.0°C -> 20 in tenths -> 0x0014
    rawBytes[0x0D] = 0x14; rawBytes[0x0E] = 0x00;
    // Accuracy: 99% representation -> 0x0063
    rawBytes[0x0F] = 0x63; rawBytes[0x10] = 0x00;
    // OEM Defined
    rawBytes[0x11] = 0x00; rawBytes[0x12] = 0x00; rawBytes[0x13] = 0x00; rawBytes[0x14] = 0x00;
    // Nominal Value: 35.0°C -> 350 in tenths -> 0x015E
    rawBytes[0x15] = 0x5E; rawBytes[0x16] = 0x01;
  ...
  public void Decode_UnknownThermalThresholds_ReturnsNullForCelsiusConversions() {
    ...
    byte[] rawBytes = new byte[0x17];
    rawBytes[0x06] = 0x42; // Status = Unknown (0x40), Location = Unknown (0x02) -> 0x42
    rawBytes[0x07] = 0x00; rawBytes[0x08] = 0x80; // Max Value: 0x8000
    rawBytes[0x09] = 0x00; rawBytes[0x0A] = 0x80; // Min Value: 0x8000
    rawBytes[0x0B] = 0x00; rawBytes[0x0C] = 0x80; // Resolution: 0x8000
    rawBytes[0x0D] = 0x00; rawBytes[0x0E] = 0x80; // Tolerance: 0x8000
    rawBytes[0x15] = 0x00; rawBytes[0x16] = 0x80; // Nominal Value: 0x8000
