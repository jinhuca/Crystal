...
namespace Crystal.UnitTests.Events
{
  public class PubSubEventFixture
  {
    ...
    public void EnsureSubscriptionListIsEmptyAfterPublishingAMessage()
    {
      ...
      Assert.True(pubSubEvent.BaseSubscriptions.Count == 0, "Subscription list is not empty");
    }
    ...
    public void EnsureSubscriptionListIsNotEmptyWithoutPublishOrSubscribe()
    {
      ...
      SubscribeExternalActionWithoutReference(pubSubEvent);
      GC.Collect();
      Assert.True(pubSubEvent.BaseSubscriptions.Count == 1, "Subscription-list is empty");
    }
    ...
    {
      TestablePubSubEvent<string> pubSubEvent = new();
    ...
    {
      var PubSubEvent = new TestablePubSubEvent<string> { SynchronizationContext = new SynchronizationContext() };
    ...
    {
      var pubSubEvent = new TestablePubSubEvent { SynchronizationContext = new SynchronizationContext() };
    ...
    public async Task ShouldNotExecuteOnGarbageCollectedFilterReferenceWhenNotKeepAlive()
    {
      ...
      wasCalled = false;
      WeakReference filterReference = new(filter);
      filter = null;
      await Task.Delay(100);
      GC.Collect();
