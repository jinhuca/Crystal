...
/// <summary>
/// Reports whether the kernel driver that ring-0 sensors depend on is present. SuperIO /
...
public static class SensorDriver {
  /// <summary>
  /// True when the PawnIO kernel driver is installed (registry check only — does not imply
  /// it can be opened by this process).
  /// </summary>
  public static bool IsInstalled => PawnIo.IsInstalled;

  /// <summary>
  /// True when the PawnIO driver device can actually be opened right now. This is the
  /// authoritative "can we read ring-0 sensors" signal: it additionally requires the driver to be
  /// running and the process to be elevated.
  /// </summary>
