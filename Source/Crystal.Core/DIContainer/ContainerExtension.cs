...
namespace Crystal
{
	public class ContainerExtension : IContainerExtension<IContainer>, IContainerInfo
	{
		private IocScopedProvider _currentScope;

		/// <summary>
		/// Gets the Default container rules used by Crystal.
		/// </summary>
		...
		{
			Instance = container;
			Instance.RegisterInstanceMany(new[] { typeof(IContainerExtension), typeof(IContainerProvider) }, this);
		...
		{
			Instance.RegisterDelegate(type, _ => factoryMethod(), Reuse.Singleton);
			return this;
		}
		...
		{
			Instance.RegisterDelegate(type, _ => factoryMethod(), Reuse.ScopedOrSingleton);
			return this;
		}
		...
		protected IScopedProvider CreateScopeInternal()
		{
			...
			_currentScope = new IocScopedProvider(resolver);
			return _currentScope;
		}

		private class IocScopedProvider : IScopedProvider
		{
			...
			public IocScopedProvider(IResolverContext resolver) => Resolver = resolver;
