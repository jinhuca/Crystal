...
namespace Crystal
{
	...
	public abstract class EventBase
	{
		...
		public virtual void Unsubscribe(SubscriptionToken token)
		{
			lock (Subscriptions)
			{
				var subscription = Subscriptions.FirstOrDefault(evt => evt.SubscriptionToken.Equals(token));
				if (subscription is { })
				{
		...
		public virtual bool Contains(SubscriptionToken token)
		{
			lock (Subscriptions)
			{
				IEventSubscription subscription = Subscriptions.FirstOrDefault(evt => ReferenceEquals(evt.SubscriptionToken, token));
