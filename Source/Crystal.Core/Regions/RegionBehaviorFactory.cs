...
namespace Crystal
{
	...
	public class RegionBehaviorFactory : IRegionBehaviorFactory
	{
		...
		private readonly Dictionary<string, Type> _registeredBehaviors = new();
		...
		public void AddIfMissing(string behaviorKey, Type behaviorType)
		{
			...
			{
				throw new ArgumentException(Format(Thread.CurrentThread.CurrentCulture, CanOnlyAddTypesThatInheritIFromRegionBehavior, behaviorType.Name), nameof(behaviorType));
			}
		...
		public IRegionBehavior CreateFromKey(string key)
		{
			if (!ContainsKey(key))
			{
				throw new ArgumentException(Format(TypeWithKeyNotRegistered, key), nameof(key));
			}
