...
public class ModuleCatalogBase : IModuleCatalog
{
  ...
  public virtual IEnumerable<IModuleInfo> CompleteListWithDependencies(IEnumerable<IModuleInfo> modules)
  {
    ...
    List<IModuleInfo> completeList = new();
  ...
  protected static string[] SolveDependencies(IEnumerable<IModuleInfo> modules)
  {
    ...
    ModuleDependencySolver solver = new();
  ...
  protected static void ValidateDependencies(IEnumerable<IModuleInfo> modules)
  {
    ...
    foreach (var moduleInfo in modules)
    {
      ...
      {
        throw new ModularityException(moduleInfo.ModuleName, Format(ModuleDependenciesNotMetInGroup, moduleInfo.ModuleName));
      }
    }
  }
  ...
  protected virtual void ValidateUniqueModules()
  {
    ...
    {
      throw new DuplicateModuleException(duplicateModule, Format(DuplicatedModule, duplicateModule));
    }
  }
  ...
  /// <returns>Collection of <see cref="IModuleInfo"/> dependents of <paramref name="moduleInfo"/>.</returns>
  protected virtual IEnumerable<IModuleInfo> GetDependentModulesInner(IModuleInfo moduleInfo)
  {
    return Modules.Where(dependentModule => moduleInfo.DependsOn.Contains(dependentModule.ModuleName));
  }
