...
namespace Crystal
{
	...
	public class ViewsCollection : IViewsCollection
	{
		...
		private readonly Dictionary<ItemMetadata, MonitorInfo> monitoredItems = new();
		...
		private void NotifyReset() => OnCollectionChanged(new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset));
		...
		private void OnItemMetadataChanged(object sender, EventArgs e)
		{
			...
			bool foundInfo = monitoredItems.TryGetValue(itemMetadata, out var monitorInfo);
			if (!foundInfo)
			{
				return;
			}
