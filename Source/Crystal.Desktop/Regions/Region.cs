	...
	public class Region : IRegion
	{
		...
		public string Name
		{
			...
			set
			{
				...
				{
					throw new InvalidOperationException(Format(CannotChangeRegionNameException, _name));
				}

				if (IsNullOrEmpty(value))
				{
					throw new ArgumentException(RegionNameCannotBeEmptyExceptionMsg);
				}
		...
		public virtual IViewsCollection Views
		{
			get
			{
				return _views ??= new ViewsCollection(ItemMetadataCollection, _ => true)
				{
		...
		public virtual IViewsCollection ActiveViews
		{
			get
			{
				_views ??= new ViewsCollection(ItemMetadataCollection, _ => true)
				{
					SortComparison = _sort
				};

				return _activeViews ??= new ViewsCollection(ItemMetadataCollection, x => x.IsActive)
				{
					SortComparison = _sort
				};
			}
    ...
    public IRegionManager Add(object view, string viewName)
		{
			...
			{
				throw new ArgumentException(Format(StringCannotBeNullOrEmpty, nameof(viewName)));
			}
		...
		private void InnerAdd(object view, string viewName, IRegionManager scopedRegionManager)
		{
			...
			if (!IsNullOrEmpty(viewName))
			{
				...
				{
					throw new InvalidOperationException(Format(RegionViewNameExistsException, viewName));
				}
		...
		private static int ViewSortHintAttributeComparison(ViewSortHintAttribute x, ViewSortHintAttribute y)
		{
			...
			{
				return y == null ? 1 : Compare(x.Hint, y.Hint, StringComparison.Ordinal);
			}
		}
	}
}
