...
public class RegionViewRegistry : IRegionViewRegistry
{
  ...
  private readonly ListDictionary<string, Func<object>> _registeredContent = new();
  ...
  private void OnContentRegistered(ViewRegisteredEventArgs e)
  {
    ...
    {
      var rootException = ex.InnerException != null ? ex.InnerException.GetRootException() : ex.GetRootException();
      throw new ViewRegistrationException(Format(OnViewRegisteredException, e.RegionName, rootException), ex.InnerException);
    }
  }
}