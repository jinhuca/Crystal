...
namespace Crystal.Unity
{
	...
	public class UnityContainerExtension : IContainerExtension<IUnityContainer>, IContainerInfo
	{
		...
		{
			Instance.RegisterFactory(type, _ => factoryMethod(), new HierarchicalLifetimeManager());
			return this;
		}
		...
		public object Resolve(Type type, params (Type Type, object Instance)[] parameters)
		{
			try
			{
				...
				ResolverOverride[] overrides = parameters.Select(p => new DependencyOverride(p.Type, p.Instance)).ToArray();
		...
		private class UnityScopedProvider : IScopedProvider
		{
			...
			public object Resolve(Type type, params (Type Type, object Instance)[] parameters)
			{
				try
				{
					DependencyOverride[] overrides = parameters.Select(p => new DependencyOverride(p.Type, p.Instance)).ToArray();
