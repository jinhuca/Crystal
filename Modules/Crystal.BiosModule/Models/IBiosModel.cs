...
/// <summary>
/// Firmware identity as a replayed static stream, plus live motherboard telemetry
/// (the BIOS tile doubles as the board's home).
/// </summary>
public interface IBiosModel {
  /// <summary>
  /// Static firmware identity, built once and replayed.
  /// </summary>
  ...
  /// <summary>
  /// Every live motherboard-category reading, for the detail sensor table.
  /// </summary>
  IObservable<IReadOnlyList<SensorReading>> BoardReadings { get; }

  /// <summary>
  /// Whether the PawnIO driver serving board sensors is installed (registry check).
  /// </summary>
  bool BoardSensorDriverInstalled { get; }

  /// <summary>
  /// Whether the PawnIO driver device can actually be opened now (installed + running +
  /// elevated) — the authoritative "board sensors are readable" signal.
  /// </summary>
