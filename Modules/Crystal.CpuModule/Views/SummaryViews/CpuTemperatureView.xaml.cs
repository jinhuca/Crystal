...
/// <summary>Temperature metric tile for the CPU summary: the temperature history graph plus the
/// live package temperature and thermal headroom (TjMax). Binds to the CPU SensorsViewModel
/// inherited from the host tile and self-registers its graph so the view model feeds it on each poll.</summary>
