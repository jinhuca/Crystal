...
namespace Crystal.TestTools.Console
{
	...
	public static class ConsoleAssert
	{
		...
		public static string Expect(string expected, Action action, bool normalizeLineEndings) =>
			Expect(expected, action, (left, right) => left == right, normalizeLineEndings ? NormalizeOptions.NormalizeLineEndings : NormalizeOptions.None);
		...
		public static string Expect(string expected, Action action, NormalizeOptions normalizeOptions = NormalizeOptions.Default) =>
			Expect(expected, action, (left, right) => left == right, normalizeOptions);
		...
		public static string Expect(string expected, Action<string[]> action, bool normalizeLineEndings = true, params string[] args) =>
			Expect(expected, () => action(args), (left, right) => left == right, normalizeLineEndings ? NormalizeOptions.NormalizeLineEndings : NormalizeOptions.None);
		...
		/// <param name="args">Args to pass to the function.</param>
		/// <param name="normalizeOptions">Options to normalize input and expected output</param>
		public static string Expect(string expected, Action<string[]> action, NormalizeOptions normalizeOptions = NormalizeOptions.Default, params string[] args) =>
			Expect(expected, () => action(args), (left, right) => left == right, normalizeOptions);
		...
		private static string Expect(string expected, Action action, Func<string, string, bool> comparisonOperator,
			NormalizeOptions normalizeOptions = NormalizeOptions.Default, string equivalentOperatorErrorMessage = "Values are not equal")
		{
			(string input, string output) = Parse(expected);
			return Execute(input, output, action, (left, right) => comparisonOperator(left, right), normalizeOptions, equivalentOperatorErrorMessage);
		}

		private static readonly Func<string, string, bool> LikeOperator = (expected, output) => output.IsLike(expected);
		...
		public static string ExpectLike(string expected, char escapeCharacter, Action action) =>
			Expect(expected, action, (pattern, output) => output.IsLike(pattern, escapeCharacter));
		...
		[Obsolete]
		public static string ExpectLike(string expected, Action action, bool normalizeLineEndings, char escapeCharacter = '\\')
		{
		...
		/// <param name="normalizeLineEndings">Whether differences in line ending styles should be ignored.</param>
		/// <param name="escapeCharacter">The escape character for the wildcard caracters.  Default is '\'.</param>
		public static string ExpectLike(string expected, Action action, NormalizeOptions normalizeLineEndings = NormalizeOptions.Default, char escapeCharacter = '\\')
			=> Expect(expected, action, (pattern, output) => output.IsLike(pattern, escapeCharacter), normalizeLineEndings, "The values are not like (using wildcards) each other");
		...
		private static string StripAnsiEscapeCodes(string input) => Regex.Replace(input, @"\u001b\[\d{1,3}m", "");
